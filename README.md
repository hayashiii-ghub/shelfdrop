...
棚に入れた項目は、もう一度ドラッグして Finder や他のアプリへ取り出せます。棚ではファイル名を変更せず、元のファイル名をそのまま表示します。
...
フッターの `Export All...` を押して保存先フォルダを選ぶと、棚に入っている全項目をまとめてコピーできます。書き出した後も元ファイルと棚の内容は残るため、繰り返し利用できます。同名ファイルがある場合は連番を付けて両方を保存します。

ファイルを元の場所から移動する操作と、全項目をひとつの ZIP ファイルにする操作も利用できます。不要になった項目はまとめてクリアできます。

### メニューバーから操作する

メニューバーの ShelfDrop アイコンから、Finder の選択項目追加、棚の表示、`Export All...`、クリア、アプリ終了を実行できます。
