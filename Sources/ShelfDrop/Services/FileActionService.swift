...
struct FileActionService {
    ...
    func chooseZipDestination() -> URL? {
        ...
        panel.nameFieldStringValue = "Archive-\(Date().compactTimestamp()).zip"
    ...
    func createZip(from items: [ShelfItem], destination: URL) throws {
        ...
        let stagingItems = stagingRoot.appendingPathComponent("Items", isDirectory: true)
        ...
        let temporaryDestination = destination.deletingLastPathComponent()
            .appendingPathComponent(".ShelfExport-\(UUID().uuidString).zip")
        defer {
